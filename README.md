...
- **Urgent — domaine `h2oufpropre45.fr` non résolu** : ce domaine ne répond à aucune requête DNS pour l'instant (vérifié `nslookup`/`curl`). Le `canonical` et le JSON-LD y pointent volontairement (bonne pratique pour le SEO une fois le domaine branché), mais `og:image`/`twitter:image` pointent temporairement vers `https://h2oufpropre.vercel.app/...` pour que l'aperçu de partage (WhatsApp/iMessage) fonctionne dès maintenant. **Dès que `h2oufpropre45.fr` est branché et résout**, remettre `og:image`/`twitter:image` sur ce domaine dans les 4 pages (`index.html`, `nettoyage-auto.html`, `canape-textile.html`, `contact.html`) et changer le `?v=2` en `?v=3` pour forcer un nouveau crawl.
- `assets/og-image.jpg` (image de partage réseaux sociaux) est désormais un raster JPG 1200×630 (photo detailing + Porsche en fond, généré à partir du vrai logo, compatible Facebook/X/WhatsApp).
